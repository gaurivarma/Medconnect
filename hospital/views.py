...
def patient_signup_view(request):
    if request.method == 'POST':
        form = forms.PatientRegistrationForm(request.POST, request.FILES)
        if form.is_valid():
            # Save the form data to create the patient object
            patient = form.save()

            # Get or create the 'patient' group
            patient_group, created = Group.objects.get_or_create(name='patient')

            # Add the user to the 'patient' group
            patient.user.groups.add(patient_group)

            # Optionally, you can also assign specific permissions to the 'patient' group, if required
            # For example: patient_group.permissions.add(permission_object)

            return redirect('patientlogin')  # Redirect to login or a success page
    else:
...
def patient_dashboard_view(request):
    ...
    # Access the doctor assigned to this patient
    doctor = patient.assignedDoctorId  # Access instance attribute
